...
#updated
def get_clinical_network(ins, n_features, n_hids, activation='relu', dropout_rate=0.5, reg_lambda=0.001):
    layers = []
    for i, n in enumerate(n_hids):
        if i == 0:
            layer = Dense(n, input_shape=(n_features,), activation=activation, kernel_regularizer=l2(reg_lambda),
                          name='h_clinical' + str(i))
        else:
            layer = Dense(n, activation=activation, kernel_regularizer=l2(reg_lambda), name='h_clinical' + str(i))

        layers.append(layer)
        layers.append(Dropout(dropout_rate, name='dropout_clinical_{}'.format(i)))

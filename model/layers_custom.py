...
#         mult = K.reshape(mult, tf.cast((-1, self.n_inputs_per_node), tf.int32))
...
class SparseTF1(Layer):
    ...
    def build(self, input_shape):
        ...
        # Use tf.convert_to_tensor instead of np.astype for better performance
        if self.map is not None:
            self.map = tf.convert_to_tensor(self.map, dtype=tf.float32)
